...
import com.rumino.exception.MeaninglessThought;
...
import com.rumino.repository.NoteRepository;
import com.rumino.exception.BadCredentialsException;
import com.rumino.util.TextValidator;
...
//        }
    ...
    private List<Note> orderNotesBySimilarity(List<Note> notes) {
        ...
        return notes.stream()
                .sorted((note1, note2) -> {
                    ...
                    float sim1 = com.rumino.util.VectorUtils.cosine(emb1, centroid);
                    float sim2 = com.rumino.util.VectorUtils.cosine(emb2, centroid);
    ...
    private List<Note> orderNotesBySubcategoryAndSimilarity(List<Note> notes, Note referenceNote) {
        ...
        return notes.stream()
                .sorted((note1, note2) -> {
                    ...
                    float sim1 = com.rumino.util.VectorUtils.cosine(emb1, referenceEmbedding);
                    float sim2 = com.rumino.util.VectorUtils.cosine(emb2, referenceEmbedding);
    ...
    private boolean isStrongCommunityMatch(Note candidate, Note referenceNote) {
        ...
        float similarity = com.rumino.util.VectorUtils.cosine(candidate.getEmbedding(), referenceNote.getEmbedding());
    ...
    private ScoredMatch findBestMatch(float[] sourceEmbedding, List<String> labels, List<float[]> candidateEmbeddings) {
        ...
        for (int i = 0; i < candidateEmbeddings.size(); i++) {
            float sim = com.rumino.util.VectorUtils.cosine(sourceEmbedding, candidateEmbeddings.get(i));
            if (sim > best) {
                secondBest = best;
                best = sim;
                bestIdx = i;
